...
qqnorm(log2(PCA_5degen_all$abs), pch = 1, frame = FALSE)
qqline(log2(PCA_5degen_all$abs), col = "steelblue")
hist(PCA_5degen_all$abs)


hist(log2(PCA_5degen_all$totalTE))
cor.test(log2(PCA_5degen_all$abs), log2(PCA_5degen_all$totalTE+0.00000000001), met0hod = "spearm", alternative = "g")

hist(log2(PCA_5degen_all$diffGC3))
hist(arcsin(PCA_5degen_all$dn))
hist(sin(PCA_5degen_all$ratioprot))
hist(log2(PCA_5degen_all$intronmeandiff))


###stats without data transformation for the responsible variable.
y1 <- glm(abs ~ totalTE + diffGC3 +dn +ratioprot +intronnrdiff+ intronmeandiff + youngold, data =PCA_5degen_all, family = quasi)
summary(y1)

###statts with data transformation

y3 <- glm(log2(abs) ~ diffGC3 +dn +totalTE + ratioprot +intronnrdiff+ intronmeandiff + youngold, data =PCA_5degen_all, family = gaussian)
summary (y3)


y3 <- glm(sqrt(abs) ~ ratioprot*youngold + totalTE*youngold + diffGC3*youngold +dn*youngold +intronnrdiff*youngold+ intronmeandiff*youngold, data = PCA_5degen_all, family=quasi) 
summary(y3)
